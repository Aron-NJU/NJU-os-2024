...
char getChar(){ // 对应SYS_READ STD_IN
	...
	//char c;
    // 假设系统调用号为 SYS_READ，读取 STD_IN
    // syscall(SYS_READ, STD_IN, (uint32_t)&c, 1, 0, 0); 第三个参数是buffer，第四个是buffersize
...
void printf(const char *format,...){
	...
	while(format[i]!=0){
		...
		if(format[i]=='%') {
			...
			switch(format[i]) {
				case 'c':
					character=va_arg(ap, char);//ap指向下一个参数的地址，然后返回当前参数的值
