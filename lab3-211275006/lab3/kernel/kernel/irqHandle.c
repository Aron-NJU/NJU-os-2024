...
void shiftProcess(struct StackFrame *sf);
...
void syscallFork(struct StackFrame *sf) {
    int pid = -1;
    // 寻找一个状态为 STATE_DEAD 的 PCB 作为新进程
    for (int i = 0; i < MAX_PCB_NUM; i++) {
    ...
    if (pid != -1) {
		...
		pcb[pid].regs.eax = 0;  // 子进程中 eax 设置为 0
        pcb[current].regs.eax = pid;  // 父进程中 eax 设置为子进程 ID 
	}
...
void syscallSleep(struct StackFrame *sf) {
    pcb[current].sleepTime = sf->ecx;  // 设置当前进程的休眠时间
    pcb[current].state = STATE_BLOCKED;  // 将当前进程状态设置为阻塞态
    shiftProcess(sf);  // 调用进程调度函数切换进程
    sf->eax = pcb[current].sleepTime;  // 返回剩余的休眠时间
    return;
}

void syscallExit(struct StackFrame *sf) {
    pcb[current].state = STATE_DEAD;  // 设置当前进程状态为死亡态
    shiftProcess(sf);  // 调用进程调度函数切换进程
    sf->eax = 0;  // 返回值为0，表示成功退出
    return;
}
